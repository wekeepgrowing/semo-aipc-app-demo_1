...
function quickstartInteractiveArgs(openclawPort) {
  return [
    "onboard",
    "--accept-risk",
    "--flow",
...
function buildWizardInteractiveCommand({ openclawPort, method }) {
  if (typeof method?.wizardAuthChoice !== "string" || !method.wizardAuthChoice.trim()) {
    throw new Error("wizard auth choice is missing");
  }
  const args = quickstartInteractiveArgs(openclawPort);
  args.push("--auth-choice", method.wizardAuthChoice.trim());
  return {
    command: "openclaw",
    args,
    maskValues: [],
...
function buildInteractiveAuthCommand({ openclawPort, method }) {
  if (!method || method.mode !== "interactive_required") {
    throw new Error("interactive_required method is required");
  }

  if (method.preferQuickstart && method.interactiveStrategy === "wizard_auth_choice") {
    return buildWizardInteractiveCommand({ openclawPort, method });
  }

  if (Array.isArray(method.interactiveCommand) && method.interactiveCommand.length > 0) {
    const fallback =
      method.interactiveStrategy === "wizard_auth_choice" ? buildWizardInteractiveCommand({ openclawPort, method }) : null;
    return {
      command: "openclaw",
  ...
  if (method.interactiveStrategy === "wizard_auth_choice") {
    return buildWizardInteractiveCommand({ openclawPort, method });
  }

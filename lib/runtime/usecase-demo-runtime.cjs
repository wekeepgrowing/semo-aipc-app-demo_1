...
const GATEWAY_TASK_TIMEOUT_MS = 90_000;
...
class UsecaseDemoRuntime {
  ...
  createUsecaseRun({
    ...
    if (definition.id === "web_research" && tasks[0]) {
      ...
      initialTranscript.push({
        ...
        role: "system",
        kind: "event",
        text: "범위 및 기준 확인 단계를 완료하고 웹 리서치 실행을 준비했어요",
        ts: now + 1,
      });
    }
    ...
    const startLog = this.appendLog(run.id, "info", "실행 탭으로 전환되어 OpenClaw 자동 실행을 시작해요");
  ...
  handleGatewayEvent({ runId, gatewayRunId, state, message, payload }) {
    ...
    const normalizedState = String(state || "running").toLowerCase();
    ...
    if (normalizedState === "error" || normalizedState === "aborted") {
      this.failAiTask(runId, currentExecution.taskId, text || "OpenClaw 실행 중 오류가 발생했어요");
      return true;
    }
    ...
    const transcriptEntry =
      normalizedState !== "running"
        ? this.appendTranscript(runId, {
            ...
            role: "system",
            kind: "event",
            text: text || `OpenClaw 상태 업데이트: ${normalizedState}`,
            ts: Date.now(),
          })
        : null;
    this.emitAll(runId, {
  ...
  async dispatchAiTask(runId, taskId) {
    ...
    if (definition.id === "web_research" && localWebResearchReady?.ready) {
      await this.dispatchLocalWebResearchTask(runId, task, run);
      return;
    }

    const taskPrompt = this.buildTaskPrompt(definition, run, task, tasks);
    const logEntry = this.appendLog(runId, "info", `OpenClaw 요청 전송: ${task.title}`);
    const transcriptEntry = this.appendTranscript(runId, {
      taskId: task.id,
      phaseId: task.phaseId,
      role: "ai",
    ...
    if (!sent.ok) {
      this.failAiTask(runId, taskId, sent.error || "OpenClaw 요청 전송 실패");
      return;
    }
    if (sent.localFallback) {
      this.failAiTask(runId, taskId, "OpenClaw gateway unavailable (local fallback blocked)");
      return;
    }
    ...
    this.timerStore.set(String(runId), [timeout]);
  }
  ...
  handleGatewayTimeout(runId, taskId) {
    ...
    this.failAiTask(runId, taskId, "OpenClaw 응답이 지연되어 확인이 필요해요");
  }
  ...
  failAiTask(runId, taskId, reason) {
    ...
    task.done = false;
    task.result = {
      error: String(reason || "OpenClaw 실행 실패"),
      failedAt: Date.now(),
    };
    tasks[index] = task;

    const whyHuman = truncateText(oneLine(reason || "OpenClaw 실행 실패"), 120);
    this.patchRunTasks(runId, tasks, {
      status: "paused",
      pauseState: {
        reason: "review",
        taskId: task.id,
        whyHuman: whyHuman || "OpenClaw 실행 실패 확인 필요",
      },
      currentExecution: null,
    });
  ...
  completeAiTaskFromGateway(runId, taskId, { responseText, payload, gatewayRunId } = {}) {
    ...
    task.result = {
      summary: compact || "OpenClaw 응답 수신",

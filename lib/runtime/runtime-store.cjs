...
function createWebResearchScopeTask(run, now) {
  ...
  return {
    ...
    result: {
      ...
      completedAt: Number(run?.createdAt || now),
    },
    review: null,
    comments: [],
  };
...
function normalizeLegacyWebResearchRunShape(run, normalized) {
  if (String(run?.usecaseId || "") !== "web_research") return normalized;
  const hasScopeTask = normalized.tasks.some(
    (task) => String(task?.phaseId || "") === "phase_scope" || String(task?.role || "") === "Human"
  );
  if (hasScopeTask) return normalized;

  const now = safeNow();
  const scopeTask = createWebResearchScopeTask(run, now);
  const nextTasks = [scopeTask, ...normalized.tasks];

  const existingPhases = Array.isArray(normalized.wbs?.phases) ? normalized.wbs.phases : [];
  const hasScopePhase = existingPhases.some((phase) => String(phase?.id || "") === "phase_scope");
  const nextPhases = hasScopePhase
    ? existingPhases.map((phase) =>
        String(phase?.id || "") === "phase_scope"
          ? {
              ...phase,
              index: 1,
              title: phase.title || "범위 및 기준 확인",
              role: phase.role || "Human",
              tasks: [scopeTask],
            }
          : phase
      )
    : [
        ...
        ...existingPhases.map((phase, index) => ({
          ...phase,
          index: index + 2,
        })),
      ];

  const nextStepsSource = Array.isArray(normalized.steps) ? normalized.steps : [];
  const nextSteps =
    nextStepsSource.length === 4 && nextStepsSource.some((step) => Number(step?.index) === 1)
      ? nextStepsSource
      : [
          { index: 1, label: "범위 및 기준 확인", status: "completed" },
          ...nextStepsSource.map((step, index) => ({
            ...step,
            index: index + 2,
          })),
        ];

  const nextStep = Number.isFinite(Number(normalized.step)) ? Math.min(Number(normalized.step) + 1, Math.max(nextSteps.length, 4)) : 2;
  ...
  return {
    ...
    tasks: nextTasks,
    wbs: {
      ...(normalized.wbs && typeof normalized.wbs === "object" ? normalized.wbs : { depth: 2 }),
      depth: 2,
      phases: nextPhases,
    },
  };
...
const CONVERSATION_ATTACHMENT_TYPES = new Set(["file"]);
...
function normalizeConversationAttachment(attachment) {
  ...
  const type = String(row.type || "file").trim().toLowerCase();
  const url = String(row.url || "").trim();
  const filename = String(row.filename || row.name || "").trim();
  const mimeType = String(row.mime || row.mimeType || mime.lookup(filename) || "application/octet-stream").trim() || "application/octet-stream";
  if (!CONVERSATION_ATTACHMENT_TYPES.has(type)) return null;
...
function normalizeConversationAttachments(raw) {
  return (Array.isArray(raw) ? raw : []).map(normalizeConversationAttachment).filter(Boolean);
}
...
function normalizeConversationMessage(message) {
  ...
  return {
    ...
    attachments: normalizeConversationAttachments(row.attachments),
...
function normalizeRunRecord(run) {
  ...
  const attachments = normalizeConversationAttachments(run.attachments);
  ...
  return normalizeLegacyWebResearchRunShape(run, normalized);
}
